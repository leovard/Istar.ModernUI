...
using Rect = Istar.ModernUI.Win32.Rect;
...
namespace Istar.ModernUI.Windows.Controls
{
        ...
        : Window
    {
        ...
        protected DpiAwareWindow()
        {
            ...
            // WmDpichanged is not send when window is minimized, do listen to global display setting changes
        ...
        {
            if (msg != NativeMethods.WmDpichanged) return IntPtr.Zero;
        ...
        private void UpdateLayoutTransform()
        {
            ...
            if (root == null) return;
            if (Math.Abs(DpiInformation.ScaleX - 1) > 0 || Math.Abs(DpiInformation.ScaleY - 1) > 0) {
        ...
        private void UpdateWindowSize(double width, double height)
        {
            ...
            if (!(Math.Abs(relScaleX - 1) > 0) && !(Math.Abs(relScaleY - 1) > 0)) return;
